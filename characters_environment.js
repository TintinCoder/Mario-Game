...
function changeStatus() {
  gameStaus = "start";
  document.getElementById('status').textContent = "Status is Starting the Game";
}
function game() {
  ...
  if (gameConfig.status === 'start') {
    ...
    textSize(40);
    textAlign(CENTER);
    text("Click on the play button to start the fun of mario game ", gameConfig.screenX / 2, gameConfig.screenY / 2);
    textSize(40);
...
function changeGameStatud(character) {
  if (noseX !== "" && gameStaus === "start" && gameConfig.status === "start") {
    world_start.play();
